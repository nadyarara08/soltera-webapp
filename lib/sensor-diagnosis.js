...
export function diagnoseCondition(avgTemperature, avgGas) {
  ...
  if (gasTier.severity === "critical") {
    return {
      ...
      explanation: `Kadar gas MQ135 rata-rata berada di atas 500 ppm (${gasTier.description.toLowerCase()}) ${
  ...
  if (gasTier.severity === "warning") {
    return {
      ...
      explanation: `Kadar gas MQ135 rata-rata berada di rentang 300–500 ppm (${gasTier.description.toLowerCase()}). ${
        tempOk
  ...
  if (gasTier.severity === "caution") {
    return {
      ...
      explanation: `Kadar gas MQ135 rata-rata berada di rentang 150–300 ppm — kemungkinan mulai terjadi pembusukan. ${
        tempOk
  ...
  if (!tempOk) {
    return {
      ...
      title: "Perlu Perhatian",
      explanation:
        "Kadar gas MQ135 masih aman (di bawah 150 ppm), tetapi suhu rata-rata berada di atas 32°C. Sistem pendingin (Peltier/kipas) kemungkinan belum bekerja maksimal — cek daya baterai, panel surya, dan sirkulasi udara pada unit.",
    };
  }
  ...
  return {
    ...
    explanation: `Suhu ruang simpan rata-rata berada di rentang aman (≤32°C) dan kadar gas MQ135 (${gasTier.description.toLowerCase()}) tidak menunjukkan tanda pembusukan selama 24 jam terakhir. Komoditas hortikultura tersimpan dalam kondisi ideal.`,
  };
}
