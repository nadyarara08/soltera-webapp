...
import { classifyGasPPM } from "@/lib/gas-thresholds";
...
export function deriveSystemStatus(reading) {
  ...
  return {
    ...
    gas: (() => {
      const tier = classifyGasPPM(gas);
      return {
        ...tier,
        // tier.label sudah berupa frasa lengkap (mis. "Intensitas Gas Normal"),
        // jadi dipakai langsung tanpa prefix tambahan di StatusBadge/SensorCard.
        label: tier.severity === "unknown" ? "Data Gas Tidak Tersedia" : tier.label,
      };
    })(),
    temperature: {
